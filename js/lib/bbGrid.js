...
(function () {
    ...
    viewOptions = ['autofetch', 'buttons', 'actions', 'colModel', 'container', 'loadDynamic',
        ...
        'onRowCollapsed', 'events', 'searchList', 'sortSequence', 'resizable', 'resizeFromBody','onRowHover','onRowLeave'];
    ...
    bbGrid.RowView = function (options) {
        this.events = {
            ...
            'mouseenter td[class!=bbGrid-actions-cell]': 'onHover',
            'mouseleave td[class!=bbGrid-actions-cell]': 'onLeave'
        };
    ...
    _.extend(bbGrid.RowView.prototype, Backbone.View.prototype, {
        ...
        template: _.template(
            '<% if (isMultiselect) {%>\
            <td class="bbGrid-multiselect-control"></td>\
            <%} if (isContainSubgrid) {%>\
        ...
        onHover: function (event) {
            this.view.trigger("rowHover", this.model, this.$el);
        },
        onLeave: function (event) {
            this.view.trigger("rowLeave", this.model, this.$el);
        },
        setSelection: function (options) {
            ...
            if (this.view.multiselect || this.view.subgrid) {
                if (this.view.multiselect) {
                    ...
                    self.selected = options.isShown || self.selected;
                    $('input[type=checkbox]', self).prop('checked', self.selected);
                    if (!self.selected && !options.isShown) {
            ...
            if (this.view.onRowHover) {
                this.view.onRowHover(this.model, options);
            }
            if (this.view.onRowLeave) {
                this.view.onRowLeave(this.model, options);
            }
        },
    ...
    _.extend(bbGrid.TheadView.prototype, Backbone.View.prototype, {
        ...
        template: _.template(
            '<% if (isMultiselect) {%>\
                <th style="width:1px" data-noresize></th>\
            <%} if (isContainSubgrid) {%>\
    ...
    _.extend(bbGrid.View.prototype, Backbone.View.prototype, {
        ...
        EventHandler: function (eventName, option1, option2) {
            ...
            case 'rowDblClick':
                ...
                break;
            case 'rowHover':
                this.onHover(option1, option2);
                break;
            case 'rowLeave':
                this.onLeave(option1, option2);
                break;
            case 'filter':
        ...
        onHover: function (model, $el){
             if (this.onRowHover) {
                this.onRowHover(model);
            }
        },
        onLeave: function (model, $el){
             if (this.onRowLeave) {
                this.onRowLeave(model);
            }
        },
        onPageChanged: function (event) {
            var $el = $(event.currentTarget),
